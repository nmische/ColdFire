...
<cfif thisTag.executionMode eq "end" or not thisTag.hasEndTag>
		...
		<cfif 
			...
			and (not StructKeyExists(attributes,"inline") or (StructKeyExists(attributes,"inline") and not attributes.inline))
			and (not StructKeyExists(attributes,"abort") or (StructKeyExists(attributes,"abort") and not attributes.abort))
			...
			<cfelse>
				<cfset QuerySetCell(data,"priority","Information",row) />
			</cfif>
			
			<!--- category --->
			<cfif StructKeyExists(attributes, "category")>
				<cfset QuerySetCell(data,"category",attributes.category,row) />
			</cfif>
			
			<!--- result --->
			<cfif IsDefined("caller.#attributes.var#")>
				<cftry>
					<cfset QuerySetCell(data,"result",coldfire_trace_udf_encode(caller[attributes.var]),row) />
					<cfcatch></cfcatch>
				</cftry>
			<cfelse>
				<cfset QuerySetCell(data,"result","(undefined)",row) />
			</cfif>
